...
public class AstroidsBiome extends ReallyinspaceModElements.ModElement {
	public static Biome biome;
	public AstroidsBiome(ReallyinspaceModElements instance) {
		super(instance, 78);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.NONE).category(Biome.Category.NONE).depth(1f).scale(2f).temperature(0.5f)
						.downfall(0f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("reallyinspace:astroids"));
			}
		}
	}
	@Override
